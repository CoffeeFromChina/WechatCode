...
import com.sptpc.service.ReaderTypeService;
...
public class ReaderTypeController {
    ...
    public String ExportExcel(@RequestParam("rdType") int rdType, @RequestParam("start") int start, HttpServletRequest request, HttpServletResponse response) throws IOException {
        int end = start + 10;
        Map<String, Integer> map = new HashMap<String, Integer>();
        map.put("start", start);
        map.put("end", end);
        ...
        try {
            response.setHeader("Content-disposition", "attachment; filename=" + "ReaderType.xls");
            ...
            Sheet sheet = new Sheet(1, 0, ReaderType.class);
            sheet.setSheetName("目录");

            List<ReaderType> typeList = readerTypeService.searchReaderType(map);

            writer.write(typeList, sheet);
            writer.finish();
            outputStream.flush();
        } catch (IOException e) {
            e.printStackTrace();
